<?php
	...
	else
	{
		...
		{	
			...
			{
				// echo "Inside else";				
				$query_2="insert into admin values('$username','$password_encrypted')";
				// echo $query_2;

				$mysqli->query($query_2);
				if($mysqli->affected_rows==1)
				{
					// echo "inside affected_rows";
					$_SESSION['admin_username'] = $username;
					setcookie('admin_username', $username, time() + (60*60*2));
					$home_url = 'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/homepage.php';
					header('Location: '.$home_url);
					exit;
				}
